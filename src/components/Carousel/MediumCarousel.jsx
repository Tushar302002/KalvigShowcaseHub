    ...
    <div className="flex items-center justify-center gap-4">
      {visibleOffsets.map((offset, idx) => {
        ...
        return (
          ...
          >
            <div className={`relative h-full ${isMain ? "p-6 rounded-[1.5625rem]" : "p-6"}`}>
              <img
                src={card.image}
                alt={card.title}
              ...
              <div className="relative z-10 flex flex-col justify-start h-full text-white">
                <div className={`flex gap-2 flex-wrap ${isMain ? "mb-4" : "mb-4"}`}>
                  {card.tags?.map((tag, idx) => (
                    <span
                      key={idx}
                      className={`font-bold bg-blue-600 text-white px-[0.5rem] py-[0.125rem] rounded-[1.25rem]
                        ${isMain ? " text-[10px] xs:text-[16px] sm:text-[1.2rem] md:text-[1.4rem] lg:text-[1.4rem] xl:text-[2rem]" : " text-[10px] xs:text-[16px]  sm:text-[0.8rem] md:text-[1.2rem] lg:text-[1rem] xl:text-[1.5rem]"}`}
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <p className={`text-gray-300 mt-4 ${isMain
